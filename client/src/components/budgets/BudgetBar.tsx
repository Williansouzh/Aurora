...
export function BudgetBar({ spentAmount = 0, limitAmount = 0 }) {
  ...
  const status = usage > 100 ? 'danger' : usage >= 70 ? 'warning' : 'good';

  return (
    <div className={cn('relative h-1.5 w-full overflow-hidden rounded-full bg-secondary', status)} aria-label={`${Math.round(usage)}% do limite`}>
      <div
        className={cn(
